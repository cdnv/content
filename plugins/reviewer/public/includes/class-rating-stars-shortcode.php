...
class RWP_Rating_Stars_Shortcode
{
	...
	public function do_shortcode_reviewer( $atts ) {
		...
		ob_start();
		echo $this->get_stars( $score, $size, $max );
	...
	public function do_shortcode_users( $atts ) {
		...
		$inline_cmp 	= $inline == 'yes' ? true : false;
		...
		if( $review_id == -1 ) { // Auto Review
			...
			//$score 		= $this->map_range( $avg, $min, $max, 0, $stars );
			$score = $avg;
			$count_label = ( $count == 1 ) ? __($this->template_field('template_users_count_label_s', true), 'reviewer') : __($this->template_field('template_users_count_label_p', true), 'reviewer');
		...
		$style = ($inline_cmp) ? 'style="display: flex; align-items: center;"' : '';
		$style2 = ($inline_cmp) ? 'style="margin-left: 10px"' : '';

		ob_start();

		echo '<div class="rwp-users-rating-shortcode" '. $style .'>';
			echo $this->get_stars( $score, $size, $max );
			echo '<div class="rwp-rating-stars-count" ' . $style2 . '>('. $count .' '. $count_label .')</div>';
		echo '</div>';
		//RWP_Reviewer::pretty_print( $this->review );
		//RWP_Reviewer::pretty_print( $this->template );
		return ob_get_clean();
