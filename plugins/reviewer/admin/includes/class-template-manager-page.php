...
class RWP_Template_Manager_Page extends RWP_Admin_Page
{
	...
	{
		// Add sections
		$sections = array( 'rwp_general_section' => __( 'General Preferences', $this->plugin_slug), 'rwp_review_section' => __( 'Review Preferences', $this->plugin_slug),  'rwp_theme_section' => __( 'Theme Preferences', $this->plugin_slug), 'rwp_colors_section' => __( 'Theme Colors', $this->plugin_slug), 'rwp_customization_section' => __( 'Customization Preferences', $this->plugin_slug), 'rwp_template_users_ratings_section' => __( 'User Reviews', $this->plugin_slug), 'rwp_template_reviews_section' => __( 'Automatic Box', $this->plugin_slug) );
	...
	public function validate_fields( $fields )
	{
		...
		foreach ($this->template_fields as $field_id => $field) {
			...
			if( $field['type'] == 'max' || $field['type'] == 'color' ) { // The field is empty
				// Nope
			} else {
				if(empty( $value )) {
					$valids[ $field_id ] = $this->template_fields[ $field_id ]['default'];
			...
			switch( $field['type'] )
			{
				...
				case 'max' :
					$valids['template_minimum_score'] = intval($fields['template_minimum_score']);

					$number = intval( $value );

					if( $number > 0 && $number > $valids['template_minimum_score'] ) {
						$valids[ $field_id ] = intval($number);
					}
					else {
		...
		if( ! empty( $errors ) ) {
			...
			add_settings_error( $this->option_name, 'rwp-template-manager-ok', __( 'Template saved, but following fields are invalid so their value is replaced with default one.', $this->plugin_slug ), 'updated' );
			add_settings_error( $this->option_name, 'rwp-template-manager-warning', implode( '<br/>', $errors ), 'update-nag' );

		} else {
			$url = admin_url('admin.php?page=reviewer-template-manager-page&template=' . $valids['template_id']);
			add_settings_error( $this->option_name, 'rwp-template-manager-ok', sprintf(__( 'Template saved. %s', $this->plugin_slug ), '<a href="'. $url .'">'. __('View', $this->plugin_slug) .'</a>'), 'updated' );
		}
	...
	public static function get_template_fields()
	{
		...
		return array(
			...
			'template_auto_reviews' => array(
				'title' 		=> __( 'Enable Automatic Box', $plugin_slug ),
				'section'		=> 'rwp_template_reviews_section',
				'default'		=>  array(),
				'description'	=> __('Skip this section if you are going to create review box manually. Choose the post types in which you want to add an reviews box. Please read the documentation for more information.', $plugin_slug ),
				'type'			=> 'posts_types'
			),
			...
			'template_auto_reviews_featured_image' => array(
				...
				'section' 		=> 'rwp_template_reviews_section',
				'default'		=> 'no',
				'description' 	=> __( "Add Post's Featured Image (if it is set) to Automatic Boxes", $plugin_slug ),
				'type'			=> 'checkbox',
			),

			'template_exclude_terms' => array(
				'title' 		=> __( 'Exclude Automatic Box from...', $plugin_slug ),
				'section'		=> 'rwp_template_reviews_section',
				'default'		=>  array(),
				'description'	=> 'Skip this setting if you are going to add reviews box manually. Choose the terms in which you want to exclude the Automatic Box.',
				'type'			=> 'posts_types'
			),

			'template_user_rating_options' => array(
				'title' 		=> __( 'User Review Fields', $plugin_slug ),
 ...
 *----------------------------------------------------------------------------*/
	...
	public function template_maximum_score_cb( $args )
	{
		...
		$min = isset($this->temp['template_minimum_score'])? $this->temp['template_minimum_score'] : 0;
		echo '<input type="hidden" name="'. $this->option_name .'[template_minimum_score]" value="'. $min .'" />';
		echo '<input type="text" name="'. $this->option_name .'[' . $field_id . ']" value="'. $value .'" placeholder="' . $default . '" />';
		echo '<span class="rwp-field-desc description">'. $description .'</span>';
	}
	...
	public function template_theme_cb( $args )
	{
		...
		for ($i=1; $i <= 9; $i++) {
			...
			echo '<li><input type="radio" id="rwp-radio-theme-'. $i .'" name="'. $this->option_name .'[' . $field_id . ']" value="rwp-theme-'. $i .'" '. $ck .' /><label title="" for="rwp-radio-theme-'. $i .'" class="rwp-theme-name" data-preview="'. RWP_PLUGIN_URL .'admin/assets/images/themes/theme-preview-'. $i .'.jpg">'. __( 'Theme', $this->plugin_slug ) .' '. $i .'</label></li>';
		}
		echo '</ul>';
		echo '<span class="rwp-field-desc description">'. $description .'</span>';
	}
	...
	public function template_colors_palettes_cb( $args )
	{
		...
		$selected = (empty( $value )) ? 'palette_forest' : $value;
